...
(async (): Promise<void> => {
    ...
    const param: ImmutableMethodParams.ImmutableGetSignableDepositParamsTS = {
        user:signer.address,
        token,
        quantity,
    };
    ...
    const withdrawal_result = await client.deposit(param);

    console.log(
        `deposit token: ${withdrawal_result}`
    );
